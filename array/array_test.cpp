...
int charArrayTest(void)
{
	const char str[] = "wonderfully watch";
	int len = strlen(str) + 1; // 和sizeof(str)值是一样的
	char *pc = new char[len];
#define LEN  sizeof(str)
	char c_array[LEN]; // 如果g++用c_array[len]可以, clang++不行
	char (*pp)[LEN];   // 如果g++用(*pp)[len]可以, clang++不行

	strcpy(c_array, str);
	strcpy(pc, c_array);
	cout << "len = " << len << endl;
	cout << "sizeof(str) = " << sizeof(str) << endl;//字符数组可以用sizeof取长度
	cout << "str = " << str << endl;
	cout << "pc = " << pc << endl;
